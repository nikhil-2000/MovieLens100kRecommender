...
from Models.GCN.compute_embeddings import CalcEmbeddings
...
class GCNMetrics(MetricBase):

    def __init__(self, dataloader, model_file, graphDataset):
        self.embedder = CalcEmbeddings(dataloader, model_file, graphDataset)

        self.embeddings, self.metadata, self.e_dict = self.embedder.get_embeddings()
        self.metadata = pd.DataFrame(self.metadata, columns=["movie_id"])

        super(GCNMetrics, self).__init__()

    def top_n_questions(self, anchor, search_size):
        # df_metadata = pd.DataFrame(metadata, columns=["problem_id", "skill_id", "skill_name"])
        key = anchor.movie_id.item()
        ...
        sorted_ids = self.metadata.iloc[sorted_indexes].movie_id.tolist()
