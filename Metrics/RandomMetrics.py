...
class RandomChoiceMetrics:
    ...
    def top_n_questions(self, anchor, search_size):
        ...
        if search_size <= len(self.movies):
            return self.movies.sample(search_size).to_list()
        else:
            metadata_size = len(self.movies)
            return self.movies.sample(metadata_size).to_list() + [0] * (search_size - metadata_size)
